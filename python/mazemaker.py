...
class Memory:
    ...
    def _get_dream_engine(self):
        ...
        if self._dream_engine_singleton is None:
            ...
            # Pick the backend that matches the live store. Previously this
            # was hard-coded to DreamEngine.sqlite — so on a PG-backed pod
            # the on-demand mazemaker_dream_* tools wrote dream_sessions /
            # dream_insights / connection_history rows into an empty
            # SQLite file at ~/.mazemaker/engine/memory.db instead of the
            # live Postgres dream_insights table. Symptom: tool returned
            # `insights=50` happily but the cluster anchors never landed
            # in PG, so Stage S synthesis (which queries dream_insights)
            # had zero clusters to work with and the dashboard's dream
            # metrics flatlined. 2026-05-21.
            backend_choice = (os.environ.get("MM_DB_BACKEND") or "").strip().lower()
            if backend_choice == "postgres":
                self._dream_engine_singleton = DreamEngine.postgres(
                    neural_memory=self._sqlite_memory
                )
            else:
